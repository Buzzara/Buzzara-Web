...
export default function Anuncio() {
  ...
  useEffect(() => {
    listarAnunciosPublicos().then(setAnuncios).catch(console.error);
  }, []);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {anuncios.map(anuncio => (
          <div key={anuncio.servicoID} className="border p-4 rounded shadow">
            ...
            <p className="text-green-700 font-bold">R$ {anuncio.preco.toFixed(2)}</p>
            <p className="text-sm text-gray-500">{anuncio.localizacao?.cidade}, {anuncio.localizacao?.estado}</p>
